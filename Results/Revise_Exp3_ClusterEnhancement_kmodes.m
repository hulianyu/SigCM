...
Enhancement_NMI_kmodes = zeros(18,RT);
Enhancement_ARI_kmodes = zeros(18,RT);
Enhancement_FSC_kmodes = zeros(18,RT);
for I=1:18
    ...
    parfor run = 1:RT
        ...
        if ~isempty(wait)
            ...
            Enhancement_ACC_kmodes(I,run) = all_metrics(1);
            Enhancement_NMI_kmodes(I,run) = all_metrics(2);
            Enhancement_ARI_kmodes(I,run) = all_metrics(4);
            Enhancement_FSC_kmodes(I,run) = all_metrics(7);
        else
...
%% NMI
Enhancement_NMI_kmodes_mean = mean(Enhancement_NMI_kmodes,2);
Enhancement_NMI_kmodes_mean_up = 100*(Enhancement_NMI_kmodes_mean-com_NMI_kmodes_mean)./com_NMI_kmodes_mean;
%
threeBars_NMI = zeros(18,3);
for I=1:18
    PLOT = [com_NMI_kmodes(I,:)' Enhancement_NMI_kmodes(I,:)'];
    threeBars_NMI(I,:) = [sum(PLOT(:,2)>PLOT(:,1)) sum(PLOT(:,2)==PLOT(:,1)) sum(PLOT(:,2)<PLOT(:,1))];
end
%% ARI
Enhancement_ARI_kmodes_mean = mean(Enhancement_ARI_kmodes,2);
Enhancement_ARI_kmodes_mean_up = 100*(Enhancement_ARI_kmodes_mean-com_ARI_kmodes_mean)./com_ARI_kmodes_mean;
%
threeBars_ARI = zeros(18,3);
for I=1:18
    PLOT = [com_ARI_kmodes(I,:)' Enhancement_ARI_kmodes(I,:)'];
    threeBars_ARI(I,:) = [sum(PLOT(:,2)>PLOT(:,1)) sum(PLOT(:,2)==PLOT(:,1)) sum(PLOT(:,2)<PLOT(:,1))];
end
%% FSC
Enhancement_FSC_kmodes_mean = mean(Enhancement_FSC_kmodes,2);
Enhancement_FSC_kmodes_mean_up = 100*(Enhancement_FSC_kmodes_mean-com_FSC_kmodes_mean)./com_FSC_kmodes_mean;
%
threeBars_FSC = zeros(18,3);
for I=1:18
    PLOT = [com_FSC_kmodes(I,:)' Enhancement_FSC_kmodes(I,:)'];
    threeBars_FSC(I,:) = [sum(PLOT(:,2)>PLOT(:,1)) sum(PLOT(:,2)==PLOT(:,1)) sum(PLOT(:,2)<PLOT(:,1))];
end
%% Plot
figure
Plot_Promote_Bars(threeBars_ACC, 'ACC (k-modes) @Enhancement');
figure
Plot_Promote_Bars(threeBars_NMI, 'NMI (k-modes) @Enhancement');
figure
Plot_Promote_Bars(threeBars_ARI, 'ARI (k-modes) @Enhancement');
figure
Plot_Promote_Bars(threeBars_FSC, 'FSC (k-modes) @Enhancement');

%% save 'Enhancement_' vars 
vars = who('Enhancement_*');
% save('Revise_Exp3_ClusterEnhancement_kmodes.mat', vars{:});