...
for (I in 10:10) {
  ...
  # Get the current random partition
  all_pi <- read.table(paste0(dataset_name, '-Random.txt'))  # Load the txt file
  ...
  for (run in 1:RT) {
    ...
    # Try to execute the jackstraw_cluster function and catch any errors
    result <- tryCatch({
      ...
      # If an error occurs, print the error and return NA for p-values
  ...
  filename_pvs <- paste0('Random_Jackstraw_all_pvs_', rowNames[I], '.txt')
  # write.table(all_pvs, file = filename_pvs, row.names = FALSE, col.names = FALSE, quote = FALSE)
  
  
  filename_time <- paste0('Random_Jackstraw_runtime_', rowNames[I], '.txt')
